...
$id_user = db_real_escape_string($conn, $_GET['id']);

// 1. Ambil id_dokter berdasarkan id_user
$get_dr = db_query($conn, "SELECT id_dokter FROM dokter WHERE id_user = '$id_user'");
$dr = db_fetch_assoc($get_dr);
...
if ($dr) {
    ...
    db_query($conn, "DELETE FROM reservasi WHERE id_jadwal IN (SELECT id_jadwal FROM jadwal_dokter WHERE id_dokter = '$id_dr')");
    
    // B. Hapus Jadwal Dokter
    db_query($conn, "DELETE FROM jadwal_dokter WHERE id_dokter = '$id_dr'");
    
    // C. Hapus Profil Dokter
    db_query($conn, "DELETE FROM dokter WHERE id_dokter = '$id_dr'");
}

// 3. Terakhir, hapus Akun di tabel Users (Induk Utama)
$hapus_user = db_query($conn, "DELETE FROM users WHERE id_user = '$id_user'");
...
if ($hapus_user) {
    ...
    echo "Gagal menghapus akun: " . db_error($conn);
}
?>