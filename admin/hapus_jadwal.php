...
if (isset($_GET['id'])) {
    ...
    db_query($conn, "DELETE FROM pembayaran WHERE id_reservasi IN (SELECT id_reservasi FROM reservasi WHERE id_jadwal = '$id_jadwal')");

    // 2. Hapus data di tabel reservasi yang menggunakan jadwal ini
    db_query($conn, "DELETE FROM reservasi WHERE id_jadwal = '$id_jadwal'");
